...
// "2026-11-30" -> "30 november 2026" (zonder tijdzone-verschuiving, anders dan formatDatum)
function formatDatumDag(s) {
  const p = parseLokaal(s);
  if (!p || !p.Y || !p.M || !p.D) return '';
  return `${p.D} ${MAANDEN[p.M - 1]} ${p.Y}`;
}
...
module.exports = {
  ...
  isoLokaal, formatDatumLang, formatDatumKort, formatDatumDag, formatTijd, toDatetimeLocal, isToekomst
};
