...
async function haal(url, { fetchImpl = fetch, alsBuffer = false } = {}) {
  ...
  try {
    ...
    if (!res.ok) {
      const fout = new Error(`HTTP ${res.status} voor ${url}`);
      fout.status = res.status; // zodat aanroepers een 404 (bestand bestaat nog niet) stil kunnen overslaan
      throw fout;
    }
...
module.exports = {
  ...
  // gedeeld met de nieuws-import
  haal, haalAfbeeldingId, stripTags,
  // voor tests
