...
// Een verwijderd geïmporteerd bericht niet opnieuw importeren.
async function negeer(bronUid) {
  await pool.query('DELETE FROM agenda_import_negeer WHERE bron = $1 AND bron_uid = $2', [NEGEER_BRON, bronUid]);
  await pool.query('INSERT INTO agenda_import_negeer (bron, bron_uid) VALUES ($1, $2)', [NEGEER_BRON, bronUid]);
}
async function negeerlijstWissen() {
  await pool.query('DELETE FROM agenda_import_negeer WHERE bron = $1', [NEGEER_BRON]);
}
async function aantalGenegeerd() {
  return (await pool.query('SELECT COUNT(*)::int AS n FROM agenda_import_negeer WHERE bron = $1', [NEGEER_BRON])).rows[0].n;
}

module.exports = {
  BRON, config, importeerNieuws, planImport, status, negeer, negeerlijstWissen, aantalGenegeerd,
  // voor tests
  leesLijst, leesArtikel, amsterdamNaarDate
};
