...
// Socialemedia-berichten (LinkedIn en Facebook) van de brigade, de club, de draagploeg en
// veteranenzaken, verzameld door de dagelijkse assistent
const LINKEDIN_BRON = 'linkedin';
const FACEBOOK_BRON = 'facebook';
const SOCIAL_BRONNEN = [LINKEDIN_BRON, FACEBOOK_BRON];
...
// "…story_fbid=411472094798370…", "…/posts/724532383492338/", "…/posts/pfbid0AbC…" -> stabiel id
function facebookBerichtId(url) {
  const s = String(url || '');
  const m = s.match(/story_fbid=(pfbid[0-9A-Za-z]+|\d{6,25})/i)
    || s.match(/\/(?:posts|videos|photos)\/(pfbid[0-9A-Za-z]+|\d{6,25})/i);
  return m ? 'fb-' + m[1] : null;
}

// Ruw socialemedia-item (LinkedIn of Facebook) uit het bestand van de assistent -> schoon record (of null).
function valideerLinkedInItem(raw) {
  ...
  let platform = null;
  if (/(^|\.)linkedin\.com$/.test(host)) platform = LINKEDIN_BRON;
  else if (/(^|\.)facebook\.com$/.test(host)) platform = FACEBOOK_BRON;
  if (!platform) return null;
  const uid = (String(raw.id || '').trim()
    || (platform === LINKEDIN_BRON ? linkedinActiviteitId(url) : facebookBerichtId(url)) || '').slice(0, 120);
  if (!uid) return null;
  const platformNaam = platform === LINKEDIN_BRON ? 'LinkedIn' : 'Facebook';
  const titel = (String(raw.titel || '').replace(/\s+/g, ' ').trim() || `Bericht op ${platformNaam}`).slice(0, 200);
  ...
  return { uid, titel, tekst, url, datum, pagina, platform, platformNaam };
}
  ...
  try {
    ...
    if (cfg.linkedin) {
      ...
      for (const raw of liItems.slice(0, 60)) {
        ...
        const bestaat = (await pool.query('SELECT id FROM nieuws WHERE bron = $1 AND bron_uid = $2', [it.platform, it.uid])).rows[0];
        if (bestaat) { r.ongewijzigd += 1; continue; }
        const inhoud = [it.tekst, `Bekijk het volledige bericht — met foto’s, video en reacties — op ${it.platformNaam}.`].filter(Boolean).join('\n\n');
        await pool.query(
          ...
          [it.titel, inhoud, it.datum || new Date(), it.pagina ? `${it.platformNaam} · ${it.pagina}` : it.platformNaam, it.platform, it.uid, it.url]
        );
        r.nieuw += 1;
      }
    }
...
module.exports = {
  BRON, LINKEDIN_BRON, FACEBOOK_BRON, SOCIAL_BRONNEN, LINKEDIN_NEGEER, config, importeerNieuws, planImport, status, negeer, negeerlijstWissen, aantalGenegeerd,
  // voor tests
  leesLijst, leesArtikel, amsterdamNaarDate, linkedinActiviteitId, facebookBerichtId, valideerLinkedInItem
};
