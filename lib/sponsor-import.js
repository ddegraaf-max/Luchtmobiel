...
async function importeer({ fetchImpl = fetch } = {}) {
  ...
  bezig = true;
  const cfg = config();
  const r = { tijdstip: new Date(), nieuw: 0, dubbel: 0, ongeldig: 0, aantalInBron: 0, assistentBijgewerkt: null, fouten: [], bron: cfg.url };
  try {
    const inbox = leesInbox(await haal(cfg.url, fetchImpl));
